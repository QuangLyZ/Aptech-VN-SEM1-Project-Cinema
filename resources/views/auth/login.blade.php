...
<div class="min-h-[calc(100vh-4rem)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative bg-cover bg-center bg-no-repeat" style="background-image: url('{{ asset('ironman.jpg') }}');">
    ...
    <div class="max-w-md w-full relative z-10">
        <!-- Form Glassmorphism -->
        <div class="space-y-8 p-10 rounded-[3rem] shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] border border-white/10 backdrop-blur-xl bg-gray-900/50">
            <div>
            ...
            <form class="mt-8 space-y-6" action="{{ route('login.post') }}" method="POST">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input id="password" name="password" type="password" autocomplete="current-password" required class="appearance-none relative block w-full px-4 pr-12 py-3 border border-white/30 bg-transparent text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-400/50 focus:border-cyan-400/50 sm:text-sm transition-all placeholder-gray-400 shadow-inner" placeholder="••••••••••••">
                            <button type="button" onclick="togglePassword('password', 'eye-icon')" class="absolute inset-y-0 right-0 pr-4 flex items-center text-cyan-400/60 hover:text-cyan-400 transition-colors">
