...
<div class="max-w-4xl mx-auto px-4 py-10 text-white">
    <div class="mt-10">
        <a href="{{ route('home') }}" class="text-blue-500 hover:text-blue-400 font-medium">
            <i class="fa-solid fa-arrow-left mr-2"></i> {{ __('ui.back_to_home') ?? 'Quay lại trang chủ' }}
        </a>
    </div>
    @if(!empty($post->thumbnail))
        <div class="mb-6">
            <img src="{{ $post->thumbnail }}" alt="{{ $post->title }}" class="w-full max-h-96 object-cover rounded">
        </div>
    @endif
